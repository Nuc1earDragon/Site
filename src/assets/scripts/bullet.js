function Bullet(X, Y, bulletPos ) {
    ...
    this.crashIcon.src="assets/img/hit1.png";
    this.speed = 2;
  }
  ...
  Bullet.prototype.getImage = function(bulletPos) {
    var src="";
    src="assets/img/bullet-" + bulletPos + ".png";
    var image= new Image();
    image.src=src;
    return image;
  };
...
function isNotSteel(j1, i1) {
   if  (map1.map1[j1][i1] !== 'S'){
       return true;
   }
    else return false;    
}
function isOnMap(bullet) {
    if ((bullet.x < 0) || (bullet.x+8> game.canvas.width) || (bullet.y < 0) || (bullet.y+8> game.canvas.height)) {
        bullet.crashed = true ;
        return false;
    }
    else return true;
